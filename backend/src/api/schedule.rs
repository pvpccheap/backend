...
/// Funció per generar el schedule de demà per totes les regles actives
/// Aquesta funció es cridarà des d'un background task a les 20:30
pub async fn generate_tomorrow_schedule(pool: &PgPool, pvpc: &PvpcClient) -> AppResult<usize> {
    let tomorrow = chrono::Local::now().date_naive() + chrono::Duration::days(1);

    // Obtenir els preus de demà
    ...
    for rule in rules {
        ...
        if (rule.days_of_week & day_bit) == 0 {
            continue; // Aquesta regla no s'aplica avui
        }
        ...
        for hour in &optimal.hours {
            ...
            sqlx::query(
                r#"
                INSERT INTO scheduled_actions (rule_id, scheduled_date, start_time, end_time, price_per_kwh)
                VALUES ($1, $2, $3, $4, $5)
